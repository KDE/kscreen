...
static std::optional<std::expected<KScreen::ModeInfo, std::string>> tryBasicModeline()
{
...
{
    const auto result = tryCvtModeline().or_else(tryBasicModeline);
    if (!result) {
        std::println(std::cerr, "Invalid custom mode: {}", arguments.join(' ').toStdString());
        return 1;
    }

    const auto modeInfo = *result;
    if (!modeInfo) {
